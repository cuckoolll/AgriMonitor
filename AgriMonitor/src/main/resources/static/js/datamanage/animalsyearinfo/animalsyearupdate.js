layui.use(['form','layer','table', 'laydate'], function(form,layer,table,laydate) {
	function init(){
		...
		if(gid){//如果有值，为更新操作
			...
			//查询数据并赋值到表单中
			$.post("/animalsyearinfo/findById", {gid:gid},function(res){
		          if(res){
	...
	function bindEvent(){
		...
		form.on('submit(submitBut)', function(data) {
			$.post("/animalsyearinfo/save", data.field,function(res){
		          if(res && res.code==0){
		        	  parent.layer.msg('保存畜牧业年度信息成功');
		        	  parent.layui.table.reload('datalist');
		        	  var index = parent.layer.getFrameIndex(window.name); //先得到当前iframe层的索引
					  parent.layer.close(index); //再执行关闭
		          }else{
		        	  layer.msg('保存畜牧业年度信息失败');
		          }
	        });
			return false;
		});
